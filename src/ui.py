...
class GuessNumberUi(ctk.CTk):
    def __init__(self):
        ...
        self.frame_settings = ctk.CTkFrame(self, width=250,height=380, corner_radius=3)
        ...
        # ---------------Status Bar (Status)---------------
        self.detail_frame = ctk.CTkFrame(self, width=250, height=120)
        self.detail_frame.grid(column=0, row=1, sticky="nsew", padx=(10, 5), pady=(0, 10))
        self.detail_frame.pack_propagate(False)
        self.my_status_title = ctk.CTkLabel(self.detail_frame, text="Status", font=("Arial", 20, "bold"))
        self.my_status_title.pack(padx=5, pady=5)
        line_game = ctk.CTkFrame(self.detail_frame, height=4)
        line_game.pack(fill="x", padx=15)
        self.my_status = ctk.CTkLabel(self.detail_frame, font=("Arial", 16),
                                      text="Hints: On \n Difficulty: Easy")
        self.my_status.pack(pady=(10, 5))
        # ---------------Right Frame (Game)---------------
        self.frame_game = ctk.CTkFrame(self, corner_radius=3)
        self.frame_game.grid(column=1, row=0,rowspan=2 ,sticky="nsew", padx=(5, 10), pady=10)
        ...
        # ---------------Tag Developer  (Game)---------------
        dev_info = ctk.CTkLabel(self.frame_game, text="Developed by MonkeyxDev", text_color=("black", "white"),
                                cursor="hand2", font=("Arial", 16))
